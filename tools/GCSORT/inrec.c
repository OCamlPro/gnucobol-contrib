...
struct inrec_t *inrec_constructor_range(int position, int length) {
    ...
    if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1020, ABEND_EXEC);
	return inrec;
}
...
struct inrec_t* inrec_constructor_range_join(int nField, int position, int length) {
	...
	if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1021, ABEND_EXEC);
...
struct inrec_t *inrec_constructor_change_position(int position, struct fieldValue_t *fieldValue) {
    ...
    if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1022, ABEND_EXEC);
	return inrec;
}
struct inrec_t *inrec_constructor_change(struct fieldValue_t *fieldValue) {
    ...
    if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1023, ABEND_EXEC);
	return inrec;
}
/*
* 
...
struct inrec_t *inrec_constructor_range_position(int posAbsRec, int position, int length) {
    ...
    if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1024, ABEND_EXEC);
	return inrec;
}
...
struct inrec_t *inrec_constructor_padding(int nAbsPos, unsigned char *chfieldValue, int nPosAbsRec) {
    ...
    /* this is error because abs position is < of current position of field */
		...
		utl_abend_terminate(MEMORYALLOC, 1025, ABEND_EXEC);
	return inrec;
}
...
struct inrec_t *inrec_constructor_subst(unsigned char *chfieldValue) {
    ...
    if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1026, ABEND_EXEC);
	return inrec;
}
...
struct inrec_t* inrec_constructor_possubstnchar(int npos, unsigned char* ntch, unsigned char* chfieldValue) {
	...
	if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1027, ABEND_EXEC);
	return inrec;
}
...
struct inrec_t *inrec_constructor_substnchar(unsigned char* ntch, unsigned char *chfieldValue) {
    ...
    if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1028, ABEND_EXEC);
	return inrec;
}
...
struct inrec_t* inrec_constructor_findrep(void)
{
	...
	if (inrec != NULL) {
		...
		utl_abend_terminate(MEMORYALLOC, 1029, ABEND_EXEC);
	return inrec;
}
		...
		switch (i->type) { 
			case INREC_TYPE_RANGE:
				...
				if (nIRangeLen == -1)		/* outrec pos, -1  (for variable)   */
					nIRangeLen = inputLength - i->range.position;
				if ((i->range.position + nIRangeLen) <= inputLength)
					memcpy(output+position+nSplitPos, input+i->range.position+nSplitPos, nIRangeLen);
				else
					/* copy only chars present in input for max len input   */
					memcpy(output+position+nSplitPos, input+i->range.position+nSplitPos, abs(inputLength - (i->range.position)));
				position+=nIRangeLen;
				break;
			case INREC_TYPE_CHANGE_POSITION:
			...
			case INREC_TYPE_CHANGE_ABSPOS:
				...
				memcpy(output + i->change.posAbsRec + nSplitPos, fieldValue_getGeneratedValue(i->change.fieldValue), fieldValue_getGeneratedLength(i->change.fieldValue));
...
/* 20160408 record input len */
				...
				if (nIRangeLen == -1)		/* outrec pos, -1  (for variable) */
					nIRangeLen = inputLength - i->range_position.position;
				
				if ((i->range_position.position+nSplitPos+nIRangeLen) <= inputLength)
                    memcpy(output+i->range_position.posAbsRec+nSplitPos, input+i->range_position.position+nSplitPos, nIRangeLen);
				else
					/* copy only char present in input for max len input    */
                    memcpy(output+i->range_position.posAbsRec+nSplitPos, input+i->range_position.position+nSplitPos, abs(inputLength - (i->range_position.position)));
			...
			case INREC_TYPE_CHANGE_CMDOPT:
				...
				if (i->changeCmd.changeCmdOpt != NULL) {
					...
					gc_memcpy(i->szChangeBufIn, input + i->range.position + nSplitPos, nIRangeLen);
					/* Nomatch with pos, len */
					if (i->changeCmd.changeCmdOpt->nomatchLen > 0) {
						memset(i->szChangeBufNoMatch, 0x00, COB_FILE_BUFF);
						gc_memcpy(i->szChangeBufNoMatch, input + i->changeCmd.changeCmdOpt->nomatchPos + nSplitPos, i->changeCmd.changeCmdOpt->nomatchLen);
					}
					...
					if ((i->range.position + nIRangeLen) <= inputLength)
						memcpy(output + position + nSplitPos, i->szChangeBufOut, nIRangeLen);
					else
						memcpy(output + position + nSplitPos, i->szChangeBufOut, nIRangeLen);
					/* copy only chars present in input for max len input   */
			...
			case INREC_TYPE_FINDREP:
			{
				...
				gc_memcpy(i->szChangeBufIn, input + i->findrepCmd.findrepCmdOpt->nStartPos + nSplitPos, nIRangeLen);
				findrep_search_replace(i->findrepCmd.findrepCmdOpt, i->szChangeBufIn, i->szChangeBufOut, nIRangeLen, nIRangeLen, input + nSplitPos, i->findrepCmd.findrepCmdOpt->nDo, 'I');
				/* save in output all data from input */
				/* DD 20250923 memcpy(output + nSplitPos + nSplit, input + nSplitPos + nSplit, globalJob->LenCurrRek); */
				memcpy(output, input, globalJob->LenCurrRek + nSplitPos);

				/* copy data modified from replace   */
				memcpy(output + i->findrepCmd.findrepCmdOpt->nStartPos + nSplitPos, i->szChangeBufOut, nIRangeLen);
...
int inrec_SetChangeCmdOpt(struct inrec_t* inrec, struct change_t* chg)
{
	if ((inrec != NULL) && (chg != NULL)) {
		...
		if (inrec->szChangeBufIn == NULL)
			utl_abend_terminate(MEMORYALLOC, 1032, ABEND_EXEC);
		inrec->szChangeBufOut = malloc(COB_FILE_BUFF);
		if (inrec->szChangeBufOut == NULL)
			utl_abend_terminate(MEMORYALLOC, 1033, ABEND_EXEC);
	}
	return 0;
}
int inrec_SetFindRepCmdOpt(struct inrec_t* Inrec)
{
	if (Inrec != NULL) {
		Inrec->szChangeBufIn = malloc(COB_FILE_BUFF);
		if (Inrec->szChangeBufIn == NULL)
			utl_abend_terminate(MEMORYALLOC, 1030, ABEND_EXEC);
		Inrec->szChangeBufOut = malloc(COB_FILE_BUFF);
		if (Inrec->szChangeBufOut == NULL)
			utl_abend_terminate(MEMORYALLOC, 1031, ABEND_EXEC);
	}
	return 0;
}
