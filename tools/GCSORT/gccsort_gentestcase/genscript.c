...
int gen_script_linux (struct params_t* params, struct key_t** pKey ) 
{
	...
	write_line(pFile, (char*)"## * File generated from gcsort_gentestcase\n");
	write_line(pFile, (char*)"## * ----------------------------------------------------------------------------\n");
	...
	for (i=0; i < n_gcsort_top; i++) {
		strcpy(pBuf, "export ");
		strcat(pBuf, szgcsort_top[i]);
		write_line(pFile, (char*)pBuf);
	}



	sprintf((char*) pBuf, (char*)"../bin/gcsort TAKE  ../take/%s.prm\n", params->szFileName);
	write_line(pFile, (char*) pBuf);
	write_line(pFile, (char*)"export RTC=$?\n");
	write_line(pFile, (char*)"else\n");
	write_line(pFile, (char*)"export RTC=1\n");
	...
	write_line(pFile, (char*)"##  checkdata sortede by GCSORT \n");
	write_line(pFile, (char*)"if [ \"$RTC\" == \"0\" ] ; then 	\n");
    ...
    i = strtol(mode, 0, 8);
    if (chmod (szFile,i) < 0) {
		fprintf(stderr,"*gcsort_gentestcase*S004* Cannot open file %s : %s\n",szFile,strerror(errno));
		return -1;
	}
	free(pBuf);
...
int gen_script_win(struct params_t* params, struct key_t** pKey )
{
	...
	sprintf((char*) pBuf, (char*)"..\\bin\\gcsort TAKE %s%s.prm\n", params->PathTake, params->szFileName);
	write_line(pFile, (char*) pBuf);
