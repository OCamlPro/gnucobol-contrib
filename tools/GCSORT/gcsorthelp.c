...
void GCSORT_Usage_all(void)
{
    ...
    printf("    GIVE same parameters of USE \n");
    printf("\n");
    printf("    SUM FIELDS = [({Pos},{Len},{FormatType2}, ...)]        [,XSUM] | [,XSUM,FNAMES=<file path | environment variable>] |\n");
    printf("                 [({Pos},{Len}, ...)],FORMAT={FormatType2} [,XSUM] | [,XSUM,FNAMES=<file path | environment variable>] |\n");
    printf("                 [NONE] | [(NONE)] [,XSUM] | [,XSUM,FNAMES=<file path | environment variable>] \n");
    printf("    XSUM without FNAMES file path generate file with same name of output file and with '.xsum' extension\n");
    printf("\n");
    printf("    RECORD [TYPE=[{V}(Variable-length)/{F}(Fixed-length)]],[LENGTH=[{len}(L1-Input record length)]\n");
    printf("                                                                ','[{len}(L2-Record length)]\n");
    printf("                                                                ','[{len}(L3-Output record length)]\n");
