...
/*  #ifdef _WIN32   */
	...
	if(INVALID_HANDLE_VALUE == hFile)
	{
		...
		fprintf(stdout,"*GCSORT*S501*ERROR: Cannot Open MMF file input : %s, %s\n", strfile, strerror(errno));
		exit(GC_RTC_ERROR);
		return 0;
	}
	...
	if(NULL == mmf->m_pbFile)  
	{
		...
		fprintf(stdout,"*GCSORT*S502*ERROR: Error Open File mmfio_Open\n");
		exit(GC_RTC_ERROR);
		return 0;
	}
	...
	*/
    ...
    if(mmf->m_pbFile == MAP_FAILED ) { 
		memcpy(mmf->m_strErrMsg, MMF_ERR_MAPVIEWOFFILE, sizeof(MMF_ERR_MAPVIEWOFFILE));
		fprintf(stdout,"*GCSORT*S503*ERROR: Error Next Map File mmfio_Open\n");
		exit(GC_RTC_ERROR);
    }
...
/* linux debug printf("mmf->m_qwFileSize %ld\n", mmf->m_qwFileSize );   */
	...
	mmf->m_pbFile = mmap( 0, mmf->m_dwBytesInView, prot, flags, mmf->m_hFile, mmf->m_nViewBegin );
   	...
   	if( mmf->m_pbFile == MAP_FAILED ) { 
		memcpy(mmf->m_strErrMsg, MMF_ERR_MAPVIEWOFFILE, sizeof(MMF_ERR_MAPVIEWOFFILE));
		fprintf(stdout,"*GCSORT*S504*ERROR: Error Open File mmfio_Open \n");
		exit(GC_RTC_ERROR);
   	}
	mmf->m_nCurPos = 0;
	mmf->m_nViewBegin = 0;
...
/* #ifdef _WIN32    */
    ...
    if(mmf->m_pbFile == MAP_FAILED ) { 
	memcpy(mmf->m_strErrMsg, MMF_ERR_MAPVIEWOFFILE, sizeof(MMF_ERR_MAPVIEWOFFILE));
	fprintf(stdout,"*GCSORT*S505*ERROR: Next Map File mmfio_SeekRead\n");
	exit(GC_RTC_ERROR);
    }
...
/* linux debug printf("		munmap ((*mmf)->m_pbFile, (*mmf)->m_dwBytesInView) ; after\n"); */
			...
			(*mmf)->m_pbFile = mmap( 0, (*mmf)->m_dwBytesInView, prot, flags, (*mmf)->m_hFile, (*mmf)->m_nViewBegin  );
		   	if((*mmf)->m_pbFile == MAP_FAILED ) { 
				memcpy((*mmf)->m_strErrMsg, MMF_ERR_MAPVIEWOFFILE, sizeof(MMF_ERR_MAPVIEWOFFILE));
				fprintf(stdout,"*GCSORT*S506*ERROR: Error Next Map File mmfio_seek\n");
				exit(GC_RTC_ERROR);
		   	}
