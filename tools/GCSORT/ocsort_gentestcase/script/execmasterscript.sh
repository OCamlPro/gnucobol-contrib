## * ----------------------------------------------------------------------------
## * File generated from gcsort_gentestcase
## * ----------------------------------------------------------------------------
## * pathgen     =../files\
## * filename    =filegen.LS
## * organization=LS
...
if [ "$RTC" == "0" ] ; then 	
EXPORT GCSORT_PATHTMP=./
EXPORT GCSORT_MEMSIZE=512000000
EXPORT GCSORT_DEBUG=0
EXPORT GCSORT_STATISTICS=2
EXPORT GCSORT_SLOT=1
EXPORT GCSORT_MLT=63
      ../bin/gcsort TAKE  ../take/filegen.LS.prm
	export RTC=$?
else
	export RTC=1
fi
##  checkdata sortede by GCSORT 
if [ "$RTC" == "0" ] ; then 	
