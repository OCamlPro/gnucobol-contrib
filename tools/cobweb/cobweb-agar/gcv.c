/*
  ...
  Modified: 2018-12-05/23:37-0500 btiffin
...
main(int argc, char** argv)
{
    ...
    if (argc > 1 && (!strcmp(argv[1], "-h") || !strcmp(argv[1], "--help"))) {
        ...
        printf("  compile a C fragment and print the value via printf\n");
        printf("  include as many headers as it takes to resolves symbols\n");
        printf("  last argument can be -V, -C, -E, -spec or %%c-of-spec\n");
        printf("\n");
        printf("  -V for COBOL VALUE statement\n");
        printf("  -C for CONSTANT AS statement\n");
        printf("  -E echo (and keep generated C files)\n");
        printf("  -spec is a string suitable for use in a printf format spec\n");
        printf("  -v, --version and -h, --help also supported\n");
        printf("\n");
        printf("  Relies on exported CFLAGS to manage include search path\n");
        return 0;
    }
    if (argc > 1 && (!strcmp(argv[1], "-v") || !strcmp(argv[1], "--version"))) {
        printf("gcv version 0.7 Dec 2018\n");
        return 0;
    }
