      ...
      *                                                                *
      *   version 1.15 Cosmetic - Add an 'e' to Curry > Currey.        *
      *                Clean up a debug block that displays the        *
      *                found VAR tables (not spaces) and is rem'd out. *
      *                                                                *
      *   version 2.16 This version will read a file containing the    *
...
160830     03  ws-Prog-Version              pic x(6) value " 2.16 ".
...
151018*     MOVE     "/var/run/mysqld/mysqld.sock" & X"00"
      *                                      TO CC-MYSQL-SOCKET.
      * Above no longer needed for v2.16
      *
...
151016              close input-file output-file
      ...
      *
       2131-Get-RDB-Params.                   *> reads file presql2.param
           ...
           End-call
      *
      * Removing the x"00" is NOT wanted for presql2 so rem'd out.
      *
      *     INSPECT WS-MYSQL-Host-Name   REPLACING ALL LOW-VALUE BY SPACE
      *     INSPECT WS-MYSQL-Implementation
      *                                  REPLACING ALL LOW-VALUE BY SPACE
      *     INSPECT WS-MYSQL-Password    REPLACING ALL LOW-VALUE BY SPACE
      *     INSPECT WS-MYSQL-Base-Name   REPLACING ALL LOW-VALUE BY SPACE
      *     INSPECT WS-MYSQL-Port-Number REPLACING ALL LOW-VALUE BY SPACE
      *     INSPECT WS-MYSQL-Socket      REPLACING ALL LOW-VALUE BY SPACE
      *
