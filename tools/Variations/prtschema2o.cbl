 ...
 program-id.    Prtschema2o.
*>              For use with MySQL (Oracle) only.
*>              { There is a separate one for Mariadb
*>                called presql2M. }
*>
*>Author.       J. C. Currey.
...
*>     the current cups spool name as is to stop the report !          *
...
*>                                                                     *
*>   WARNING : <<<<<<<<<<<< ------------- >>>>>>>>>>>>>>>>>>           *
*>        This version after running through presql will create a very *
*>        large TD-COLUMNS table as it has three fields namely:        *
*>         CB-COLUMN-DEFAULT                                           *
*>         CB-COLLATION-NAME                                           *
*>         CB-COLUMN-COMMENT    of longtext that is defined as         *
*>          400,000,000 chars each and is too long for GunCobol if not *
*>           any others.  So they have been changed to x(4096) which   *
*>            is long enough for the column table.                     *
*>                                                                     *
*>                                                                     *
*>          2.12 - For signed comp values now has a test to check if   *
*>                 unsigned is set in ws-ca-column-type (WS-I) & if so *
*>                 removes the sign 'S' see low in 2142-FIELD-LOOP.    *
*>                 sames as in presql2M & presql2o.                    *
*>                 forgot to document this.                            *
*>                 20160830    10 November 2016                        *
*>**********************************************************************
*>
*> COPYRIGHT NOTICE.
*>*****************
...
*>***************************************************
 01  ws-Name-Program                     pic x(16)     value "prtschema2o 2.12".
   *> needed 4 read-params call.
 01  ws-parm-prog-name                   pic x(10) value "prtschem2o".
*>
...
*>
       ...
       01  TD-COLUMNS.
           ...
           05  CB-COLUMN-DEFAULT                 PIC X(1024000).
           ...
           05  CB-COLUMN-TYPE                    PIC X(1024000).
           05  CB-COLUMN-KEY                     PIC X(3).
           05  CB-EXTRA                          PIC X(30).
           05  CB-PRIVILEGES                     PIC X(80).
           05  CB-COLUMN-COMMENT                 PIC X(1024).
           05  CB-GENERATION-EXPRESSION          PIC X(1024000).
*>  /MYSQL-END\
*>***************************************************************
*>                Procedure Division                            *
*>***************************************************************
     ...
     evaluate ws-ca-data-type
       ...
       when "tinyint"
              ...
              if ws-A = zero
                    ...
                    move "PIC  9(XX) COMP" to ws-ca-picture
              end-if
 *>*>            move "PIC S9(XX) COMP" to ws-ca-picture
...
*>
     ...
     write    print-record from ws-heading-buffer after 1 lines.
